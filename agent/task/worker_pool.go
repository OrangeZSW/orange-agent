...
func (wp *WorkerPool) executeSubTask(subTask *domain.SubTask) {
	...
	// 获取agent执行任务
	agent, err := wp.agentManager.GetDefaultAgent()
	if err != nil {
		subTask.Status = domain.StatusFailed
		subTask.Error = fmt.Sprintf("获取agent失败: %v", err)
	...
	response, err := agent.Chat(wp.ctx, messages)
	if err != nil {
		subTask.Status = domain.StatusFailed
		subTask.Error = fmt.Sprintf("任务执行失败: %v", err)
		logger.Error(subTask.Error)
		return
	}

	// 添加助手响应到上下文
	wp.contextManager.AddMessage(subTask.ID, "assistant", response.Content, len(response.Content))
